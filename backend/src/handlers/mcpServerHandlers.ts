...
export async function handleMcpServerCreate(
  ...
  logger.log(
    "McpServer",
    "Create",
    `建立 MCP Server: ${server.id} (${server.name})`,
  );
...
export async function handleMcpServerUpdate(
  ...
  if (!existing) {
    emitNotFound(
      connectionId,
      WebSocketResponseEvents.MCP_SERVER_UPDATED,
      "MCP Server",
      mcpServerId,
      requestId,
    );
  ...
  const updated = mcpServerStore.update(
    mcpServerId,
    name,
...
export async function handleMcpServerRead(
  ...
  if (!server) {
    emitNotFound(
      connectionId,
      WebSocketResponseEvents.MCP_SERVER_READ_RESULT,
      "MCP Server",
      mcpServerId,
      requestId,
    );
...
export const handleMcpServerDelete = createDeleteHandler({
  ...
  deleteConfig: {
    ...
    findPodsUsing: (canvasId, mcpServerId) =>
      podStore.findByMcpServerId(canvasId, mcpServerId),
    deleteNotes: (canvasId, mcpServerId) =>
      mcpServerNoteStore.deleteByForeignKey(canvasId, mcpServerId),
...
const mcpServerBindHandler = createBindHandler({
  ...
  podStoreMethod: {
    bind: (canvasId, podId, mcpServerId) =>
      podStore.addMcpServerId(canvasId, podId, mcpServerId),
  },
...
export const handlePodUnbindMcpServer = withCanvasId<PodUnbindMcpServerPayload>(
  ...
  async (
    connectionId: string,
    canvasId: string,
    payload: PodUnbindMcpServerPayload,
    requestId: string,
  ): Promise<void> => {
    ...
    logger.log(
      "McpServer",
      "Unbind",
      `從 Pod ${podId} 解綁 MCP Server ${mcpServerId}`,
    );
