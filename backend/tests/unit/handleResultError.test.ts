...
describe('handleResultError', () => {
	describe('success: false 時', () => {
		it('應呼叫 emitError 並回傳 true', () => {
			...
			const isError = handleResultError(result, CONNECTION_ID, EVENT, REQUEST_ID, FALLBACK_ERROR);
		...
		it('有傳入 errorCode 時應使用該 errorCode', () => {
			...
			handleResultError(result, CONNECTION_ID, EVENT, REQUEST_ID, FALLBACK_ERROR, 'NOT_FOUND');
		...
		it('未傳入 errorCode 時應 fallback 至 INTERNAL_ERROR', () => {
			...
			handleResultError(result, CONNECTION_ID, EVENT, REQUEST_ID, FALLBACK_ERROR);
		...
		it('result.error 為 undefined 時應使用 fallbackError', () => {
			const result = { success: false, error: undefined } as unknown as Result<string>;
