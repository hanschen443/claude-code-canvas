...
vi.mock("@/components/icons/SlackIcon.vue", () => ({
  default: {
    name: "SlackIcon",
    template: '<svg data-provider="slack" />',
    props: ["size"],
  },
}));
vi.mock("@/components/icons/TelegramIcon.vue", () => ({
  default: {
    name: "TelegramIcon",
    template: '<svg data-provider="telegram" />',
    props: ["size"],
  },
}));
vi.mock("@/components/icons/JiraIcon.vue", () => ({
  default: {
    name: "JiraIcon",
    template: '<svg data-provider="jira" />',
    props: ["size"],
  },
}));

async function mountComponent(provider: string) {
  const { default: ChatIntegrationBlockedHint } =
    await import("@/components/integration/ChatIntegrationBlockedHint.vue");
...
describe("ChatIntegrationBlockedHint", () => {
  ...
  describe("Telegram provider", () => {
    it("應顯示 Telegram 的封鎖提示文字", async () => {
      ...
      expect(wrapper.text()).toContain(
        "此 Pod 已連接 Telegram，訊息由 Telegram 驅動",
      );
    });
  });
