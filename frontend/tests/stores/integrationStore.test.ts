...
function createMockIntegrationApp(
  ...
  beforeEach(async () => {
    ...
    mockTransformApp.mockImplementation((raw: Record<string, unknown>) => {
      const resources = Array.isArray(raw.resources)
        ? (raw.resources as Array<{ id: string; name: string }>).map((r) => ({
            id: r.id,
            label: "#" + r.name,
          }))
        : [];
      return createMockIntegrationApp({
        ...
        connectionStatus:
          (raw.connectionStatus as IntegrationApp["connectionStatus"]) ??
          "disconnected",
        resources,
      });
    });
  ...
  describe("getters", () => {
    ...
    describe("connectedAppsByProvider", () => {
      it("應過濾出 connectionStatus 為 connected 的 apps", () => {
        const store = useIntegrationStore();
        const connectedApp = createMockIntegrationApp({
          id: "app-1",
          connectionStatus: "connected",
        });
        const disconnectedApp = createMockIntegrationApp({
          id: "app-2",
          connectionStatus: "disconnected",
        });
        const errorApp = createMockIntegrationApp({
          id: "app-3",
  ...
  describe("actions", () => {
    ...
    describe("createApp", () => {
      it("應呼叫 INTEGRATION_APP_CREATE 並回傳轉換後的 app", async () => {
        ...
        mockBuildCreatePayload.mockReturnValueOnce({
          name: "New App",
          botToken: "xoxb-test",
        ...
        const result = await store.createApp("slack", {
          name: "New App",
          botToken: "xoxb-test",
          signingSecret: "secret",
        });
    ...
    describe("bindToPod", () => {
      it("應呼叫 POD_BIND_INTEGRATION 並帶正確 payload", async () => {
        ...
        const store = useIntegrationStore();
        mockBuildBindPayload.mockReturnValueOnce({
          appId: "app-1",
          resourceId: "channel-1",
        });
        mockCreateWebSocketRequest.mockResolvedValueOnce({
      ...
      it("帶 extra 參數時應傳遞到 buildBindPayload", async () => {
        ...
        expect(mockBuildBindPayload).toHaveBeenCalledWith(
          "app-1",
          "user-123",
          extra,
        );
    ...
    describe("removeAppFromEvent", () => {
      ...
      it("provider 不存在時不應拋出錯誤", () => {
        ...
        expect(() =>
          store.removeAppFromEvent("unknown", "app-1"),
        ).not.toThrow();
      });
    ...
    describe("refreshAppResources", () => {
      ...
      it("app 不存在時不應拋出錯誤", async () => {
        ...
        await expect(
          store.refreshAppResources("slack", "app-999"),
    ...
    describe("updateAppStatus", () => {
      it("應更新對應 app 的 connectionStatus", () => {
        ...
        expect(store.getAppsByProvider("slack")[0]?.connectionStatus).toBe(
          "connected",
        );
      });
      ...
      it("傳入 resources 時應同時更新 resources", () => {
        ...
        expect(store.getAppsByProvider("slack")[0]?.resources).toEqual([
          { id: "ch-1", label: "#general" },
        ]);
      });
      ...
      it("找不到 app 時不應拋出錯誤", () => {
        ...
        expect(() =>
          store.updateAppStatus("slack", "nonexistent", "connected"),
        ).not.toThrow();
      });
      ...
      it("provider 不存在時不應拋出錯誤", () => {
        ...
        expect(() =>
          store.updateAppStatus("unknown", "app-1", "connected"),
        ).not.toThrow();
      });
    });
  });
